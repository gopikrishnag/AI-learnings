...
with project_client:
    ...
    #upload a file
    file = project_client.agents.upload_file_and_poll(file_path='ScrumGlossaryTerms.pdf', purpose=FilePurpose.AGENTS)
    print(f"Uploaded file, file ID: {file.id}")

    # create a vector store with the file you uploaded
    vector_store = project_client.agents.create_vector_store_and_poll(file_ids=[file.id], name="Books_vectorstore")
    ...
    message_file = project_client.agents.upload_file_and_poll(file_path='OOPs.pdf', purpose=FilePurpose.AGENTS)
    ...
    message = project_client.agents.create_message(
        thread_id=thread.id, role="user", content="What is an Abstract class? what is Daily Scrum?", attachments=[attachment]
    )
